...
def handle_rate_limit(response):
    if response.status_code == 429:
        wait_time = float(
            response.headers.get("Retry-After", response.json().get("wait", 1))
        )
...
def positions_map(session):
    ...
    data = api_request(
        session, "GET", "securities"
    )  # after switching /positions to /securities, no error popup.
    if data is None:
...
def accept_active_tender_offers(session):
    ...
    if offers[0]["is_fixed_bid"]:
        ...
        resp = api_request(
            session, "POST", f"tenders/{tender_id}", params={"price": price}
        )
...
def step_once(session):
    ...
    accept_active_tender_offers(
        session
    )  # Automatically checking and acceptting all of the tender offer
    ...
    return (
        ...
        edge2,
        {
            ...
            "bear_bid": bear_bid,
            "bear_ask": bear_ask,
            "ritc_bid_usd": ritc_bid_usd,
            "ritc_ask_usd": ritc_ask_usd,
            "usd_bid": usd_bid,
            "usd_ask": usd_ask,
            "ritc_bid_cad": ritc_bid_cad,
            "ritc_ask_cad": ritc_ask_cad,
        },
    )
    ...
    with requests.Session() as s:
        ...
        while status == "ACTIVE" and not shutdown:
            try:
                ...
                print(
                    f"tick={tick} e1={e1:.4f} e2={e2:.4f} ritc_ask_cad={info['ritc_ask_cad']:.4f}"
                )
                sleep(LOOP_SLEEP)
