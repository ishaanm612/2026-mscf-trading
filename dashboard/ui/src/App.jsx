...
function ChartFrame({ title, description, rows, symbols, margin }) {
  ...
  return <article className="option-chart"><h3>{title}</h3><p>{description}</p><div className="chart chart-small"><ResponsiveContainer><ComposedChart data={rows} margin={{ top: 14, right: 18, bottom: 4, left: 2 }}><CartesianGrid stroke="#d9e3ee" strokeDasharray="3 3" /><XAxis dataKey="tick" type="number" domain={[0, 300]} ticks={[0, 100, 200, 300]} tick={{ fill: '#41516a', fontSize: 11 }} /><YAxis width={58} tickFormatter={percent} tick={{ fill: '#41516a', fontSize: 11 }} domain={['auto', 'auto']} /><Tooltip content={<TradeTooltip />} /><ReferenceArea y1={latest?.analyst - margin} y2={latest?.analyst + margin} fill="#dcfce7" fillOpacity={0.4} /><Line isAnimationActive={false} dataKey="analyst" stroke="#168043" strokeWidth={2} dot={false} name="Analyst IV" />{symbols.map((symbol) => <Line key={symbol} isAnimationActive={false} dataKey={symbol} stroke={optionColor(symbol)} strokeWidth={2} dot={false} name={symbol} />)}{symbols.map((symbol) => <Line key={symbol + '-trade'} isAnimationActive={false} dataKey={(row) => row.trades?.[symbol] ? row[symbol] : null} stroke="none" dot={{ r: 6, fill: '#9333ea', stroke: '#ffffff', strokeWidth: 2 }} activeDot={false} name={symbol + ' trade'} />)}</ComposedChart></ResponsiveContainer></div></article>
}
